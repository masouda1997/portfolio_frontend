   ...
   useLayoutEffect(()=>{
	...
	return (
		<div className=" flex flex-row-reverse w-screen overflow-hidden ">
			<header className=" flex justify-center items-end flex-col gap-3 p-5 relative  basis-2/12">
               ...
               >
					...
					<li 
                  className="  flex items-center justify-center self-end "
                  onMouseEnter={() =>setMenuHover((prev) => ({ ...prev, user: true }))}
                  onMouseLeave={()=> setMenuHover(prev=>({...prev , user:false}))}
               >
						<Link
							style={{ border: `${primaryColor} 1px solid` }}
							className="p-2 rounded-full  flex items-center justify-center self-end"
					...
					<li 
                  className=" flex items-center justify-center self-end "
                  onMouseEnter={() =>setMenuHover((prev) => ({ ...prev, portfolio: true }))}
                  onMouseLeave={()=> setMenuHover(prev=>({...prev , portfolio:false}))}
               >
						<Link
							style={{ border: `${primaryColor} 1px solid` }}
							className="p-2 rounded-full  flex items-center justify-center self-end"
